...
factor = exogeneous_variables.loc[:, ['rf_rate', 'mkt_ptf', 'smb', 'hml', 'mom']] # start_date:
...
predictive = exogeneous_variables.loc[:, ['1M_Tbill_yield', 'div_yield_mkt_ptf', 'term_spread', 'default_spread']]# start_date:
...
mutual_fund = mutual_fund[mutual_fund['fdate'].isin(common_dates)]
...
mutual_fund_returns = mutual_fund.reset_index()
mutual_fund_returns = mutual_fund_returns.pivot_table(index='fundname', columns='fdate', values='rdt', aggfunc='first')
...
weighted_portfolio["Excess Returns"] = weighted_portfolio["Returns"] - factor["rf_rate"]
